...
class WebCrawlingSpider:
    def __init__(self):
        ...
        self.target_city_list = ['Manchester', 'Liverpool',
                                 'Blackpool', 'Chester', 'Northwich', 'Bolton', 'Warrington']
    ...
    def get_page_html(self, page_url: str) -> bs4.BeautifulSoup:
        ...
        random_user_agent_header['User-Agent'] = self.user_agent_list[random.randint(
            0, len(self.user_agent_list) - 1)]
    ...
    def get_company_names_and_links(self, page_url: str) -> dict[str, str]:
        ...
        company_names = wait.until(EC.visibility_of_all_elements_located(
            (By.XPATH, "//span[@class='text-xl font-medium text-black']")))
    ...
    def get_number_of_pages(self, page_url: str) -> int:
        ...
        j = driver.find_elements(
            By.XPATH, "//button[@class='outline-none flex items-center justify-center w-10 h-10 border border-surface-variant-light-outline rounded-lg text-fiber-neutral-500 border-fiber-neutral-500 hover:bg-white hover:border-fiber-grey-900 focus:bg-white focus:border-fiber-grey-900']")
        driver.quit()
        self.spider_sleep()
    ...
    def get_specific_company_bcorp_information(self, company_page_url: str) -> dict[str, str]:
        ...
        overview_div = driver.find_element(
            By.XPATH, "//div[@class='lg:hidden flex flex-col bg-gray-light p-4 space-y-4']")
        ...
        for section in section_div_list:
            ...
            print(f"section:\n{section}\n\n")
            heading = section.find_element(By.XPATH, "./span").get_attribute('innerHTML')
            # heading = section.find_element(By.TAG_NAME, 'span')
            print(f"found heading:\n{heading}\n")

            if heading == "Operates In":
                special_xpath = "./div/p"
            else:
                special_xpath = "./p"
            ...
            info_text = info_div.find_element(By.XPATH, special_xpath).get_attribute('innerHTML')
    ...
    def write_bcorp_company_information(self):
        ...
        for target_city_url in target_city_urls:
            ...
            for page_number in range(number_of_pages):
                ...
                target_city_company_name_link_dictionary = self.get_company_names_and_links(
                    target_city_url)
                print(
                    f"updating cumulative company list with the following companies and links:\n{target_city_company_name_link_dictionary.keys()}.\n")
                all_target_city_company_name_link_dict.update(
                    target_city_company_name_link_dictionary)
